...
function PostListItem({ post }: { post: BlogPostItem }) {
 ...
 */
function PostNewsCard({ post }: { post: BlogPostItem }) {
  const locale = useLocale();
  const primaryTag = post.tags[0];
  const colorClass =
...
export function BlogList({ posts }: { posts: BlogPostItem[] }) {
  ...
  const tagNameMap = useMemo(() => {
    ...
    for (const p of posts) {
      p.tagIds.forEach((id, i) => {
        if (map[id] === undefined) map[id] = p.tags[i] ?? id;
      });
    }
    return map;
  }, [posts]);
